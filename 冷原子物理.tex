...
\textbullet\
量子数（Quantum Number）：由于沿着 \( \hat{z} \) 方向的旋转对称性，总量子数 $ F_{z}^{1}+F_{z}^{2}+L_z $ 是守恒的。这里 $ F_{z}^{i=1,2} $ 分别是这两个原子超精细自旋的 \( z \)-分量，\( L_z \) 是它们相对角动量的 \( z \)-分量。为了简化，我们只考虑开通道和闭通道中的 \( s \)-波态，这时 \( L_z = 0 \)。在这种简化下，开通道的 $ F_{z}^{1}+F_{z}^{2} $ 应该等于闭通道的 $ F_{z}^{1}+F_{z}^{2} $。例如，对于 \( ^6\text{Li} \)，如果将开通道设定为 \( |a\rangle \otimes |b\rangle \)，总的 \( F_z = 0 \)，那么总共有五种组合具有总 \( F_z = 0 \)，其他四种分别是 \( |a\rangle \otimes |d\rangle \)，\( |b\rangle \otimes |e\rangle \)，\( |c\rangle \otimes |f\rangle \)，和 \( |d\rangle \otimes |e\rangle \)。

\textbullet\
闭通道vs开通道（Closed versus Open）：当两个原子相距较远时，开通道与闭通道之间的能量差由塞曼能量决定，通常这个能量差远高于动能。因此，当两个原子自开通道的低能散射态发生碰撞时，它们无法散射到闭通道的散射态。这就是这些通道被称为“闭通道”的原因。如图\ref{fig:2.5}(a)的插图所示，当选择 \( |a\rangle \otimes |b\rangle \) 作为开通道时，其他四种组合都可以被视为闭通道。

\textbullet\
能量可调控性（Energy Tunability）：通常当开通道被选择为低激发的超精细自旋态时，随着磁场的增加，开放通道的能量相对于闭通道下降。因此可以设想，随着磁场的增加，散射阈值可以从上方接近闭通道中的束缚态。


\textbullet\
通道耦合（Coupling between Channels）：当两个原子彼此接近时，它们之间的原子间势主要依赖于两个原子的电子自由度。这里，由于每个碱金属原子都有一个电子，原子间势依赖于它们的总电子自旋是单态还是三重态（实际上，范德瓦尔斯相互作用对于电子自旋单态和三重态是相同的，但短程排斥相互作用则依赖于电子自旋。），图\ref{fig:2.5}(a)中给出了一个例子。例如，考虑开通道 \( |a\rangle \otimes |b\rangle \)，它们的电子自旋在磁场作用下被极化，且它们的总电子自旋更接近三重态。然而，超精细耦合会使得电子自旋单态成分混合进来。因此，短程势会耦合不同的通道，尽管这种耦合通常较弱。

通过讨论上述两个通道的特性，我们可以考虑一个简化的耦合通道模型，来示范磁场如何改变散射长度。该模型的示意图如图 \ref{fig:2.6}(a) 所示。主要的考虑因素如下：
...
\begin{equation}
	...
	\label{eq:2.53}
\end{equation}
\eqref{eq:2.52}式来源于闭通道的波函数在 \( r = r_0 \) 处为零，\eqref{eq:2.53}式确定了开通道散射波函数中的相移。这里我们假设 \( \chi_{\pm} \) 与能量无关，且 \( r_0 \) 的值很小。这两个等式给出：
\begin{equation}
